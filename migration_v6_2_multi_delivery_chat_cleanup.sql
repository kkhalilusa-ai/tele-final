-- Telegram Store Bot v6.2
-- Atomic multi-item instant delivery, persistent chat cleanup state and
-- validated Custom Emoji feature controls.
...
ALTER TABLE user_ui_state ADD COLUMN IF NOT EXISTS last_menu_message_id BIGINT;
ALTER TABLE user_ui_state ADD COLUMN IF NOT EXISTS keyboard_initialized BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE user_ui_state ADD COLUMN IF NOT EXISTS keyboard_signature TEXT;
ALTER TABLE user_ui_state ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
...
GRANT ALL ON user_ui_state TO service_role;
...
BEGIN
  ...
  -- Fast idempotent replay. A key cannot be reused for another product,
  -- quantity or user, and a partial historical delivery is never returned.
  ...
  IF FOUND THEN
    ...
    END IF;
    SELECT
      array_agg(i.payload_ciphertext ORDER BY i.id),
      array_agg(i.payload_iv ORDER BY i.id),
      array_agg(i.payload_auth_tag ORDER BY i.id)
    INTO v_ciphertexts, v_ivs, v_tags
    FROM product_inventory_items i
    WHERE i.order_id = v_order.id AND i.status = 'sold';
    ...
    END IF;
    RETURN QUERY SELECT
      v_order.id, v_order.product_name, v_order.quantity, v_order.unit_price,
  ...
  -- The user lock serializes retries for one customer. The second idempotency
  -- check converts simultaneous double-clicks into a replay of the first order.
  SELECT * INTO v_user
  FROM users u
  ...
  -- Product metadata and stock policy are locked for the complete purchase.
  ...
  IF v_product.bulk_pricing_enabled THEN
    ...
    FROM bulk_pricing_tiers t
    WHERE t.product_id = v_product.id
      AND t.min_quantity <= p_quantity
  ...
  IF v_product.fulfillment_type = 'instant' THEN
    -- Lock exactly p_quantity unique inventory rows. Locked rows from another
    -- transaction are skipped and can never be delivered twice.
    SELECT array_agg(selected.id ORDER BY selected.id), count(*)::INTEGER
    ...
    IF COALESCE(v_item_count, 0) <> p_quantity THEN
  ...
  ELSE
    ...
    IF NOT v_product.unlimited_stock THEN
      UPDATE products
      SET manual_stock = GREATEST(manual_stock - p_quantity, 0),
  ...
  UPDATE users
  SET wallet_balance = wallet_balance - v_total, updated_at = NOW()
  WHERE id = v_user.id AND wallet_balance >= v_total
  ...
  IF v_product.fulfillment_type = 'instant' THEN
    ...
    WHERE product_inventory_items.id = ANY(v_item_ids)
      AND product_inventory_items.product_id = v_product.id
      AND product_inventory_items.status = 'available'
      AND product_inventory_items.order_id IS NULL;
  ...
  INSERT INTO wallet_transactions(
    ...
    v_user.id, 'purchase', -v_total, v_balance, 'order', v_order.id::TEXT, v_product.name
  );
