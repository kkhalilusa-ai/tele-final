...
(function () {
  ...
  async function renderUsers(){var state=stateFor('users');state.sort=state.sort||'created_at';state.direction=state.direction||'desc';var response=await api('/users'+queryString(state));clear(content);var sortValue=state.sort+'|'+state.direction;content.appendChild(node('div',{class:'toolbar'},toolbarSearch(state,'Telegram ID, username, first name',renderCurrent),selectControl('Sort',sortValue,[['created_at|desc','Newest users'],['created_at|asc','Oldest users'],['wallet_balance|desc','Highest balance'],['wallet_balance|asc','Lowest balance']],function(v){var parts=v.split('|');state.sort=parts[0];state.direction=parts[1];state.page=1;renderCurrent();}),selectControl('Language',state.language||'',[['','All'],['en','English'],['ar','Arabic'],['hi','Hindi']],function(v){state.language=v;state.page=1;renderCurrent();}),selectControl('Status',state.suspended||'',[['','All'],['false','Active'],['true','Suspended']],function(v){state.suspended=v;state.page=1;renderCurrent();})));var rows=response.data.map(function(row){return[node('td',{},node('span',{class:'cell-main',text:row.username||row.first_name||'Unnamed'}),node('span',{class:'cell-sub',text:'#'+row.id})),node('td',{class:'mono',text:String(row.telegram_id)}),node('td',{text:row.language}),node('td',{class:'mono',text:money(row.wallet_balance)}),node('td',{},badge(row.is_suspended?'suspended':'active')),node('td',{text:formatDate(row.created_at)}),node('td',{},node('div',{class:'row-actions'},button('Details','ghost small',function(){showUser(row.id);}),button(row.is_suspended?'Unsuspend':'Suspend',row.is_suspended?'success small':'danger small',function(){suspendUser(row);})))];});content.appendChild(panel('Users','Balances are stored transactionally; open Details to add or deduct wallet balance',node('div',{},table(['User','Telegram ID','Language','Balance','Status','Joined',''],rows),pagination(response.meta,function(page){state.page=page;renderCurrent();}))));}
